...
//    uint32_t sensorVal = quickSampleIR();
  ...
  val2 = float(sampleBuffer[1]);
  val3 = float(sampleBuffer[2]);
  

  sendToPC(&val1, &val2, &val3); // Send 3 values
//  sendToPC(&val1);
  
  printSensorOLED(0, sampleBuffer[0]); // Use function below to print sensor values to OLED screen
  printSensorOLED(1, sampleBuffer[1]); // Use function below to print sensor values to OLED screen
  printSensorOLED(2, sampleBuffer[2]); // Use function below to print sensor values to OLED screen
//  Serial.println(val);
