...
int sleepMillis = 957; // milliseconds for sleep timer to run
...
// MAX: 65535ms
  timer.setTimer(sleepMillis);// milliseconds
...
void loop() {
  ...
  ********************************************************/
  ...
  unsigned long sleepDuration = myMillis - preMillis;
  unsigned long loopDuration = myMillis - oldMillis;
  oldMillis = myMillis; // update oldMillis
  myTime = Teensy3Clock.get(); // Read current time from the Teensy rtc
